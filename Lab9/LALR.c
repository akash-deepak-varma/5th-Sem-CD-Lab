...
}; // Array to store first of each variable
char follow[26][255] = {
  {
    0
  }
}; // Array to store follow of each variable
char *
  var, * term;
char start;
...
int main(int argc, char
  ...
  // for the derivation S -> A, S is nullable if either S gives a null directly(in some other derivation) or if A is nullable
  int no_change = 0;
  do {
    no_change = 0;
    for (int i = 0; i < n; i++) {
  ...
  // if the first character of a derivation is a terminal then we have found our first, else we find first of the first non-nullable variable which will the first of the lhs also
  do {
    no_change = 0;
    for (int i = 0; i < n; i++) {
      if (a[i].der[0] != '@') {
  ...
  int stack[500], top = -1;
  ind = 0;
  // Parse the word to see if it can be derived from the given LALR grammer
  ...
  while (1) {
    ...
    if (tab[stack[top]][ff].op == 'S') {
      ...
      stack[top + 1] = term[ff];
      stack[top + 2] = tab[stack[top]][ff].state_no;
      top += 2;
      ind++;
